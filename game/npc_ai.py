...
def get_eligible_targets(state: GameState, actor_idx: int, phase: GamePhase) -> list[int]:
    ...
    match phase:
        case GamePhase.NIGHT_GUARD:
            # Guard can protect anyone alive (including self) except last night's target
            eligible = alive_indices.copy()
            if state.last_guard_target is not None:
                # If the last guarded player is alive, exclude them (no consecutive guard)
                if state.last_guard_target in eligible:
                    eligible.remove(state.last_guard_target)
            return eligible
...
def choose_night_action(state: GameState, player_idx: int, phase: GamePhase) -> int | None:
    ...
    match phase:
        case GamePhase.NIGHT_GUARD if player.role == Role.GUARD:
            # Guard prioritises the sheriff if alive and valid (not self, not last target)
            alive = state.players.get_alive_players()
            sheriff = [p for p in alive
                       if p.is_sheriff and p.index != player_idx
                       and p.index != state.last_guard_target]
            if sheriff:
                return sheriff[0].index
            # Otherwise protect from eligible targets excluding self
            non_self = [t for t in targets if t != player_idx]
            return random.choice(non_self) if non_self else (random.choice(targets) if targets else None)
...
def choose_vote_target(state: GameState, player_idx: int) -> int | None:
    ...
    Strategy depends on the player's personality trait:
    - PARANOID: votes for whoever got most votes last round, or high-index players
    - TRUSTING: follows whoever the sheriff is likely to suspect, or no-confidence
    - ANALYTICAL: spreads votes, avoids bandwagons
    - IMPULSIVE: random votes, high variance
    - LOYAL: always votes with the pack (werewolf: follows wolf pack;
             villager: follows current vote leader)
    - RANDOM: truly random choice
    """
    from game.player import Personality

    # ── PK (runoff) mode: voters must choose from tied candidates only ──
    if state.pk_mode:
        if not state.pk_candidates:
            return None
        # Filter to only living PK candidates (excluding self)
        pk_options = [t for t in state.pk_candidates
                      if state.players.get_player(t) and state.players.get_player(t).alive
                      and t != player_idx]
        if not pk_options:
            return None
        # In PK mode, vote simply: randomly pick one of the tied candidates
        return random.choice(sorted(pk_options))
    ...
    # ── Werewolf voting — always bandwagon, but with personality nuance ──
    if player.role == Role.WEREWOLF:
        ...
        if non_ww:
            ...
            # No existing wolf votes: pick with personality twist
            if personality == Personality.ANALYTICAL:
                # Analytical wolves target special roles first (high-value targets)
                from game.roles import Team
                specials = [p for p in non_ww if p.alive and p.role.team == Team.VILLAGE
                            and p.role != Role.VILLAGER]
                if specials:
                    return random.choice([p.index for p in specials])
            elif personality == Personality.PARANOID:
                # Paranoid wolves target the most recent voter or high-index player
                high_idx = [p for p in non_ww if p.alive and p.index >= 8]
                if high_idx:
                    return random.choice([p.index for p in high_idx])
            elif personality == Personality.IMPULSIVE:
                # Impulsive wolves: 30% self-vote (as a joke/prank), else random
    ...
    # ── Village voting — personality-driven ──
    if personality == Personality.PARANOID:
        # Vote for whoever got the most votes last round (via game log)
        # Fallback: vote for a high-index player
        high_idx_targets = [t for t in targets if t >= 8]
        if high_idx_targets:
            return random.choice(high_idx_targets)
        return random.choice(targets)

    elif personality == Personality.TRUSTING:
        # Follow the sheriff's presumed target
        # Find the sheriff among alive players
        sheriff = next((p for p in state.players.players if p.is_sheriff and p.alive), None)
        if sheriff and sheriff.index in state.votes:
            return state.votes[sheriff.index]
        # No sheriff vote yet: vote for whoever has most votes (consensus)
        vote_counts: dict[int, int] = {}
        for voter, target in state.votes.items():
            vote_counts[target] = vote_counts.get(target, 0) + 1
        if vote_counts:
            max_v = max(vote_counts.values())
            leaders = [t for t, c in vote_counts.items() if c == max_v]
            return random.choice(leaders)
        return random.choice(targets)

    elif personality == Personality.ANALYTICAL:
        # Avoid bandwagons — pick someone who has 0 or 1 votes
        vote_counts = {}
        for voter, target in state.votes.items():
            vote_counts[target] = vote_counts.get(target, 0) + 1
        underdogs = [t for t in targets if vote_counts.get(t, 0) <= 1]
        if underdogs:
            return random.choice(underdogs)
        return random.choice(targets)

    elif personality == Personality.IMPULSIVE:
        # High variance: 30% chance to self-vote (as a joke), else random
        if random.random() < 0.30:
            return player_idx
        return random.choice(targets)

    elif personality == Personality.LOYAL:
        # Follow the pack: vote for whoever has the most votes
        vote_counts = {}
        for voter, target in state.votes.items():
            vote_counts[target] = vote_counts.get(target, 0) + 1
        if vote_counts:
    ...
    # RANDOM personality or fallback
    return random.choice(targets)
